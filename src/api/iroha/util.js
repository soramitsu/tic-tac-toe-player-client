...
export const GRPC_PROXY_ADDRESS = 'http://localhost:8081'
...
export function newCustomCommandService () {
  return new CommandService(GRPC_PROXY_ADDRESS)
}

export function newCustomQueryService () {
  return new QueryService(GRPC_PROXY_ADDRESS)
}

export function newCustomCommandServiceOptions (privateKeys, creatorAccountId, quorum) {
  return {
    privateKeys: privateKeys,
    quorum: quorum,
    creatorAccountId: creatorAccountId,
    commandService: newCustomCommandService(),
    timeoutLimit: DEFAULT_TIMEOUT_LIMIT
  }
}

export function newCustomQueryServiceOptions (privateKey, creatorAccountId) {
  return {
    privateKey: privateKey,
    creatorAccountId: creatorAccountId,
    queryService: newCustomQueryService(),
    timeoutLimit: DEFAULT_TIMEOUT_LIMIT
  }
}
